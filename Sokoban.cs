...
public class Sokoban : MonoBehaviour
{
    ...
    public GameObject wall; // 壁のスプライト
    ...
    {
        //カメラの追従
        camera.transform.position = new Vector3(player.transform.position.x - 2, player.transform.position.y, 10);
    ...
    private void CreateStage()
    {
        ...
        for (int y = 0; y < rows; y++)
        {
            for (int x = 0; x < columns; x++)
            {
                ...
                {
                    GameObject sr = Instantiate(wall);
                    // タイルの位置を設定
                    sr.transform.position = GetDisplayPosition(x, y);
                }
            }
        }
